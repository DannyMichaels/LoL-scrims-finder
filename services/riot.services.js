...
const RIOT_API_BASE_URL =
  process.env.RIOT_TOURNAMENT_STUB === 'true'
...
const createTournamentProvider = async (region, callbackUrl) => {
  try {
    ...
    if (!VALID_REGIONS.includes(upperRegion)) {
      ...
      throw new Error(
        `Unsupported region: ${region}. Please use one of: ${VALID_REGIONS.join(
          ', '
        )}`
      );
    }
    ...
    const payload = {
      ...
      url:
        callbackUrl ||
        `${process.env.API_URL || 'http://localhost:3000'}/api/riot/callback`,
    };

    console.log(
      'Creating tournament provider with payload:',
      JSON.stringify(payload, null, 2)
    );
    console.log(
      `Using regional endpoint: ${riotApi.defaults.baseURL}/providers`
    );

    const response = await riotApi.post('/providers', payload);

    console.log(
      'Tournament provider created successfully. Provider ID:',
      response.data
    );
...
const createTournamentCode = async (tournamentId, region, options = {}) => {
  try {
    ...
    console.error(
      'Error creating tournament code:',
      error.response?.data || error.message
    );
    const errorMessage =
      error.response?.data?.message ||
...
const updateTournamentCode = async (tournamentCode, region, updates) => {
  try {
    ...
    console.error(
      'Error updating tournament code:',
      error.response?.data || error.message
    );
    const errorMessage =
      error.response?.data?.message ||
...
const setupRiotTournamentForScrim = async (scrimData) => {
  try {
    ...
    console.log(
      'Step 1: Creating tournament provider for region:',
      upperRegion
    );
    ...
    console.log(
      'Step 3: Creating tournament code for tournament ID:',
      tournamentId
    );
    const tournamentCodes = await createTournamentCode(
      tournamentId,
      upperRegion,
      {
        count: 1,
